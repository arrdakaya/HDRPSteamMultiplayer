...
public class TrapPlacer : NetworkBehaviour
{
            ...
            if (Physics.Raycast(ray, out hitInfo, distance, PlayerLayer))
            {
                if (!isCooldown4)
                {
                    ...
                    if (Input.GetMouseButtonDown(1) && isPlayerBlind == false)
                    {
                        ...
                        {
                            blindPlayer = hitInfo.transform;
                            isCooldown4 = true;
                            blindImage.fillAmount = 1;
                            CmdBlindPlayer(blindPlayer);
                        }
                    }
                }
                else
    ...
    void RPCPlaceObject(Vector3 position, Quaternion hitRotation)
    {
        if (isServer)
        {
            InstantiatedObject = Instantiate(parasiteSpawn, new Vector3(position.x, position.y, position.z), hitRotation);
    ...
    [Command]
    void CmdBlindPlayer( Transform blindPlayer)
    {
        blindPlayer.GetChild(0).GetComponent<Volume>().enabled = true;
        isPlayerBlind = true;
        UniTask.Delay(2000);
        blindPlayer.GetChild(0).GetComponent<Volume>().enabled = false;
        UniTask.Delay(10000);
        isPlayerBlind = false;
    }
}
